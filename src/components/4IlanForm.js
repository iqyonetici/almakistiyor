...
export default function IlanForm({ open, onClose, onSubmit, user, kategoriAgaci }) {
  ...
  // birOncekiAdim ve onClose her render'da değişebildiği için aşağıdaki useEffect'in
  // popstate handler'ı içinde her zaman en güncel halini çağırmak üzere ref'lerde tutuyoruz.
  ...
  // Panel açıkken tarayıcı geçmişine "guard" bir giriş ekleniyor; donanım geri tuşu bu girişi
  // tüketince (popstate) paneli kapatmak yerine formun bir önceki adımına/kademesine dönüyoruz
  // ve guard girişini yeniden ekliyoruz ki bir sonraki geri tuşu da yakalanabilsin.
  // Panel kapanmadıkça mevcut sayfanın/kategorinin kendi URL state'ine (router.push vb.) dokunulmuyor.
  // Panel programatik olarak (✕ / Vazgeç / Tamam butonlarıyla) kapanırsa, guard girişi
  // tüketilmeden kalmasın diye cleanup'ta bir kerelik history.back() ile geri alınır.
  useEffect(() => {
    if (!open || typeof window === 'undefined') return

    let guardAktif = true
    let kapatilirkenGeriAlinacak = true
    window.history.pushState({ ilanFormGuard: true }, '')
    alert('GUARD EKLENDI, history.length=' + window.history.length)

    function handlePopstate() {
      alert('POPSTATE TETIKLENDI, guardAktif=' + guardAktif + ', step=' + step + ', katYol.length=' + katYol.length)
      if (!guardAktif) return
      const devamEtti = birOncekiAdimRef.current()
      if (devamEtti) {
        // Geri tuşu bir history girişini tükettiği için yenisini ekleyip dinlemeye devam ediyoruz.
        window.history.pushState({ ilanFormGuard: true }, '')
      } else {
        // Gidecek yer kalmadı: paneli kapat. Guard girişi popstate ile zaten tüketildi,
        // cleanup'ta tekrar history.back() çağırmaya gerek yok.
        guardAktif = false
        kapatilirkenGeriAlinacak = false
        onCloseRef.current()
      }
    }
    ...
    return () => {
      ...
      // Panel popstate dışında bir yolla (✕ / Vazgeç / Tamam) kapandıysa,
      // bizim eklediğimiz guard girişi tarayıcı geçmişinde fazlalık olarak kalmasın diye geri alınır.
      if (guardAktif && kapatilirkenGeriAlinacak) {
        window.history.back()
      }
      guardAktif = false
    }
