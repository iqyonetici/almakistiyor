import Link from 'next/link'
import Head from 'next/head'
import Navbar from '../../../components/Navbar'
import Footer from '../../../components/Footer'
import SeoMeta from '../../../components/SeoMeta'
import { kategorileriGetir } from '../../../lib/kategoriDB'
import { ilanListele } from '../../../lib/db'
import { sehirSlugtanIsimBul, sehirIsimdenSlugBul } from '../../../lib/sehirler'
import { supabase } from '../../../lib/supabase'
import styles from '../[slug].module.css'

const ANA_KATEGORILER = ['emlak', 'vasita', 'ikinci-el-sifir-alisveris', 'is-makineleri', 'hizmetler', 'ozel-ders', 'is-ilanlari', 'hayvanlar', 'yedek-parca-aksesuar-donanim-tuning']

// kategori/[slug].js'teki slugBul ile birebir aynı mantık (tek kaynaktan sapmamak için)
function slugBul(liste, hedefSlug, yol) {
  for (const kat of (liste || [])) {
    const yeniYol = yol.concat([kat])
    if (kat.slug === hedefSlug) return yeniYol
    if (kat.altKategoriler && kat.altKategoriler.length > 0) {
      const sonuc = slugBul(kat.altKategoriler, hedefSlug, yeniYol)
      if (sonuc) return sonuc
    }
  }
  return null
}

export async function getServerSideProps({ params }) {
  const { slug, sehir: sehirSlug } = params

  const sehirIsim = sehirSlugtanIsimBul(sehirSlug)
  if (!sehirIsim) {
    // Geçersiz şehir slug'ı (81 il listesinde yok) -> 404
    return { notFound: true }
  }

  const kategoriAgaci = await kategorileriGetir()
  const yol = slugBul(kategoriAgaci, slug, [])

  if (!yol || yol.length === 0) {
    return { notFound: true }
  }

  const node = yol[yol.length - 1]
  const isAltKat = !ANA_KATEGORILER.includes(slug)
  const aktifAnaKat = yol[0]?.slug || null

  const filtreTip = node.filtre_tip || null
  const ozelKolonFiltreleri = ['emlak_tip', 'islem_turu', 'marka']
  const ozelKolon = ozelKolonFiltreleri.includes(filtreTip) ? filtreTip : null
  const ozelDeger = ozelKolon ? (node.filtre_deger || null) : null
  const markaFiltre = (ozelKolon === 'marka' && aktifAnaKat === 'vasita') ? ozelDeger : null

  // kategori/[slug].js ile aynı sorgu mantığı + sehir filtresi eklendi
  // (ilanListele zaten sehir parametresini destekliyor, db.js değişmedi)
  const { data } = await ilanListele({
    kategori: (!isAltKat) ? slug : (ozelKolon && aktifAnaKat) ? aktifAnaKat : undefined,
    altKategori: (isAltKat && !ozelKolon) ? slug : undefined,
    anaKatSlug: aktifAnaKat || undefined,
    emlakTip: ozelKolon === 'emlak_tip' ? ozelDeger : undefined,
    islemTuru: ozelKolon === 'islem_turu' ? ozelDeger : undefined,
    marka: markaFiltre || undefined,
    sehir: sehirIsim,
    limit: 24,
  })

  // Thin content engeli: bu şehir+kategori kombinasyonunda ilan yoksa sayfa üretme
  if (!data || data.length === 0) {
    return { notFound: true }
  }

  // "Diğer şehirler" linki: aynı kategoride, bu şehir hariç, ilan olan
  // şehirleri çek (thin-content kuralı burada da geçerli — sadece gerçek
  // ilanı olan şehirler listelenir). db.js değişmedi, ayrı/bağımsız sorgu.
  let digerSehirler = []
  if (supabase) {
    let q = supabase
      .from('ilanlar_pro')
      .select('sehir')
      .eq('durum', 'aktif')
      .eq('onay_durumu', 'onaylandi')
      .neq('sehir', sehirIsim)
      .not('sehir', 'is', null)

    if (isAltKat && !ozelKolon) {
      if (aktifAnaKat) q = q.eq('kategori', aktifAnaKat)
    } else if (!isAltKat) {
      q = q.eq('kategori', slug)
    } else if (aktifAnaKat) {
      q = q.eq('kategori', aktifAnaKat)
    }

    const { data: sehirSatirlari } = await q.limit(500)
    const benzersizIsimler = [...new Set((sehirSatirlari || []).map(r => r.sehir).filter(Boolean))]
    digerSehirler = benzersizIsimler
      .map(isim => ({ isim, slug: sehirIsimdenSlugBul(isim) }))
      .filter(s => s.slug)
      .sort((a, b) => a.isim.localeCompare(b.isim, 'tr'))
      .slice(0, 12)
  }

  return {
    props: {
      slug,
      sehirSlug,
      sehirIsim,
      kategoriLabel: node.label || slug,
      ilanlar: data,
      digerSehirler,
    },
  }
}

export default function SehirKategoriSayfasi({ slug, sehirSlug, sehirIsim, kategoriLabel, ilanlar, digerSehirler }) {
  const baslik = `${sehirIsim} ${kategoriLabel} İlanları`
  const aciklama = `${sehirIsim}'de ${kategoriLabel} kategorisinde ${ilanlar.length} talep ilanı. Alıcılar ne aradığını yazdı, satıcılar burada bulabilir.`

  return (
    <>
      <SeoMeta
        title={baslik}
        description={aciklama}
        canonical={`/kategori/${slug}/${sehirSlug}`}
        ogType="website"
      />

      <Head>
        <script
          type="application/ld+json"
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              '@context': 'https://schema.org',
              '@type': 'BreadcrumbList',
              itemListElement: [
                { '@type': 'ListItem', position: 1, name: 'Ana Sayfa', item: 'https://almakistiyor.com/' },
                { '@type': 'ListItem', position: 2, name: kategoriLabel, item: `https://almakistiyor.com/kategori/${slug}` },
                { '@type': 'ListItem', position: 3, name: sehirIsim, item: `https://almakistiyor.com/kategori/${slug}/${sehirSlug}` },
              ],
            }),
          }}
        />
      </Head>

      <Navbar />
      <div className={styles.sayfa}>
        <div className="container">
          <nav className={styles.breadcrumb}>
            <Link href="/">Ana Sayfa</Link> <span>›</span>{' '}
            <Link href={`/kategori/${slug}`}>{kategoriLabel}</Link> <span>›</span>{' '}
            <span>{sehirIsim}</span>
          </nav>

          <h1 className={styles.baslik}>{sehirIsim} {kategoriLabel} İlanları</h1>
          <p className={styles.altBaslik}>{ilanlar.length} talep ilanı bulundu</p>

          <div className={styles.liste}>
            {ilanlar.map((ilan) => {
              const sehirIlce = [ilan.sehir, ilan.ilce].filter(Boolean).join(' / ')
              const fiyat = ilan.fiyat_min && ilan.fiyat_max
                ? `₺${Number(ilan.fiyat_min).toLocaleString('tr-TR')} – ₺${Number(ilan.fiyat_max).toLocaleString('tr-TR')}`
                : null
              return (
                <Link key={ilan.id} href={`/ilan/${ilan.id}`} className={styles.kart}>
                  <h3>{sehirIlce ? `${sehirIlce} — ` : ''}{kategoriLabel} arıyorum</h3>
                  {fiyat && <span className={styles.fiyat}>{fiyat}</span>}
                  {ilan.aciklama && <p>{ilan.aciklama.slice(0, 120)}</p>}
                </Link>
              )
            })}
          </div>

          {digerSehirler.length > 0 && (
            <>
              <p className={styles.altBaslik}>Diğer şehirlerde {kategoriLabel}</p>
              <nav className={styles.breadcrumb}>
                {digerSehirler.map((s, i) => (
                  <span key={s.slug}>
                    <Link href={`/kategori/${slug}/${s.slug}`}>{s.isim}</Link>
                    {i < digerSehirler.length - 1 && ' · '}
                  </span>
                ))}
              </nav>
            </>
          )}

          <div className={styles.tumFiltreler}>
            <Link href={`/?kategori=${slug}&sehir=${encodeURIComponent(sehirIsim)}`} className="btn-primary">
              Tüm filtrelerle görüntüle →
            </Link>
          </div>
        </div>
      </div>
      <Footer />
    </>
  )
}
